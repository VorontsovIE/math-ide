...
class InputHandler:
        ...
        while True:
            try:
                choice = self.console.input(
                    f"Выберите вариант (1-{len(param_def.options)}): "
                )
    ...
    def get_user_choice(self, prompt: str, choices: Optional[List[str]] = None) -> str:
        ...
        while True:
            try:
                ...
                self.console.print(
                    f"[red]Допустимые варианты: {', '.join(choices)}[/red]"
                )
        ...
        while True:
            try:
                ...
                if min_value <= choice <= max_value:
                    ...
                    self.console.print(
                        f"[red]Введите число от {min_value} до {max_value}[/red]"
                    )
    ...
    def get_transformation_choice(self, num_transformations: int) -> Optional[int]:
        ...
        while True:
            try:
                choice = self.console.input(
                    f"Выберите преобразование (1-{num_transformations}, 0-выход): "
                )
                ...
                if 0 <= choice_num < num_transformations:
                    ...
                    self.console.print(
                        f"[red]Выберите число от 1 до {num_transformations}[/red]"
                    )
            except ValueError:
                self.console.print("[red]Введите корректное число[/red]")
            except KeyboardInterrupt:
    ...
    def get_rollback_choice(self, num_steps: int) -> Optional[int]:
        ...
        while True:
            try:
                choice = self.console.input(
                    f"К какому шагу откатиться? (1-{num_steps}, 0-отмена): "
                )
    ...
    def get_problem_input(self) -> Optional[str]:
        ...
        try:
            problem = self.console.input(
                "\n[bold]Введите математическую задачу:[/bold] "
            ).strip()
            if not problem:
