...
class SolutionProcessor:
    ...
    def process_solution_step(self, problem: str) -> bool:
        ...
        try:
            ...
            transformations_result = (
                self.transformation_generator.generate_transformations(current_step)
            )
            if not transformations_result or not transformations_result.transformations:
                self.display_manager.show_error(
                    "Не удалось сгенерировать трансформации"
                )
                return False

            # Display transformations and get user choice
            self.display_manager.show_transformations(
                transformations_result.transformations
            )
            choice = self.input_handler.get_transformation_choice(
                len(transformations_result.transformations)
            )
            ...
            if selected_transformation.requires_user_input:
                parameters = self._collect_transformation_parameters(
            ...
            self.display_manager.show_info(
                f"Применено преобразование: {selected_transformation.description}"
            )
    ...
    def _collect_transformation_parameters(
        ...
        self.display_manager.show_info(
            f"Трансформация '{transformation.description}' требует параметры:"
        )
        ...
        for param_def in transformation.parameter_definitions:
            try:
                if param_def.param_type == ParameterType.NUMBER:
                    ...
                    self.display_manager.show_error(
                        f"Неизвестный тип параметра: {param_def.param_type}"
                    )
                    return None
                ...
                self.display_manager.show_error(
                    f"Ошибка при вводе параметра {param_def.name}: {e}"
                )
                return None
