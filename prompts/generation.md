...
- Все 5 преобразований должны быть результатом применения **ОДНОГО** действия к **ИСХОДНОМУ** выражению A
- НЕ создавай цепочку преобразований, где каждое опирается на предыдущее
- НЕПРАВИЛЬНАЯ ЛОГИКА: (1) A→B, (2) B→C, (3) C→D
- ПРАВИЛЬНАЯ ЛОГИКА: (1) A→B, (2) A→C, (3) A→D
- ПРАВИЛЬНАЯ ЗАПИСЬ: (1) B, (2) C, (3) D

**СТРОГО ЗАПРЕЩЕНО:**
- Создавать последовательность преобразований, где каждое зависит от предыдущего
- Применять несколько преобразований подряд в одном пункте
- Использовать результат предыдущего преобразования как основу для следующего

**Пример набора преобразований в форме цепочки для cos^2(x) = sin(2x). НЕ пиши цепочки, а только отдельные преобразования:**
❌ (1) cos^2(x) = 2sin(x)cos(x)
❌ (2) cos(x) = 2sin(x)
❌ (3) 2sin(x)/cos(x) = 1
❌ (4) sin(x)/cos(x) = 1/2
❌ (5) tan(x) = 1/2
...
✅ (1) \\frac{{1}}{{2}}cos^2(x) = \\frac{{1}}{{2}}\\sin(2x)              // разделить обе части на 2
✅ (2) cos^2(x) = 2sin(x)cos(x)         // применить известное тождество sin(2x) = 2sin(x)cos(x)
✅ (3) 1 - sin^2(x) = sin(2x)           // применить известное тождество cos^2(x) = 1 - sin^2(x)
✅ (4) cos^2(x) = sin(2x)cos(x)         // умножить обе части на cos(x)
✅ (5) cos^2(x) + 1 = sin(2x) + 1       // прибавить 1 к обеим частям
✅ (6) \\frac{{cos^2(x)}}{{cos(x)}} = \\frac{{sin(2x)}}{{cos(x)}}         // разделить обе части на cos(x)
✅ (7) cos^2(\\sqrt{{x^2}}) = sin(2x) \\text{{ or }} cos^2(-\\sqrt{{x^2}}) = sin(2x)       // заменить в левой части x на \\sqrt{{x^2}}
✅ (8) cos^2(y/2) = sin(2 y/2)           // заменить x на y/2
✅ (9) cos^2(x) = sin(x + x)             // представить 2x как x + x
...
**4. ИСПОЛЬЗУЙ ТОЛЬКО ИЗВЕСТНЫЕ ПРЕОБРАЗОВАНИЯ**
...
- Отдавай предпочтение базовым алгебраическим операциям и основным тригонометрическим тождествам

**Примеры ИЗВЕСТНЫХ преобразований:**
✅ Раскрытие скобок: a(b+c) = ab + ac
✅ Приведение подобных: 2x + 3x = 5x
✅ Перенос слагаемых: x + 2 = 5 → x = 5 - 2
✅ Умножение/деление обеих частей на число
✅ Основные тригонометрические тождества: sin²(x) + cos²(x) = 1, sin(2x) = 2sin(x)cos(x)

**Примеры НЕИЗВЕСТНЫХ преобразований (НЕ используй):**
❌ Специализированные формулы для интегралов и производных
❌ Редкие тригонометрические тождества
❌ Сложные алгебраические тождества
❌ Формулы из высшей математики (интегралы, производные, ряды)
...
10. **Используют результат предыдущего преобразования** как основу для следующего
11. **Применяют несколько преобразований подряд** в одном пункте
12. **Создают логическую последовательность** преобразований, где каждое зависит от предыдущего
13. **Используют редкие или неизвестные формулы** (специализированные тождества, сложные алгебраические формулы)
14. **Применяют формулы из высшей математики** (интегралы, производные, ряды)
...
- **НЕ используй юникод-символы** (², ³, ±, →, ≤, ≥, ≠, ∞, π, θ, α, β, γ, δ, ε, φ, ψ, ω, Δ, Σ, Π, ∫, ∂, ∇, √, ∛, ∜)
- **ИСПОЛЬЗУЙ ТОЛЬКО LaTeX-КОМАНДЫ**: ^2 вместо ², ^3 вместо ³, \pm вместо ±, \leq вместо ≤, \geq вместо ≥, \neq вместо ≠, \infty вместо ∞, \pi вместо π, \theta вместо θ, \alpha вместо α, \beta вместо β, \gamma вместо γ, \delta вместо δ, \epsilon вместо ε, \phi вместо φ, \psi вместо ψ, \omega вместо ω, \Delta вместо Δ, \Sigma вместо Σ, \Pi вместо Π, \int вместо ∫, \partial вместо ∂, \nabla вместо ∇, \sqrt{{}} вместо √, \sqrt[3]{{}} вместо ∛, \sqrt[4]{{}} вместо ∜
...
2. **Количество вариантов**: Предложи хотя бы 5 различных преобразований.
...
❌ **НЕПРАВИЛЬНО**: "Заменить 1 на sin^2(x) + cos^2(x)" - усложняет без необходимости
...
❌ **НЕПРАВИЛЬНО**: "cos²(x) = sin(2x)" - использование юникод-символа ² вместо ^2
...
7. **Проверь независимость**: убедись, что преобразование НЕ зависит от результатов других преобразований
8. **Проверь отсутствие цепочки**: убедись, что все преобразования независимы друг от друга
9. **Проверь известность**: убедись, что используются только известные и часто применяемые формулы
10. **Проверь LaTeX-синтаксис**: убедись, что используются только LaTeX-команды, а не юникод-символы
